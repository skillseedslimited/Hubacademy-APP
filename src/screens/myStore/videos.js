...
class MyStoreVideoScreen extends React.Component {
  ...
  getVideos = async () => {
    let token = await AsyncStorage.getItem('userToken');
      if (token !== null) {
        ...
        fetch(setup.endPoint+"/store/purchases",{
            method: 'get',
            headers: {
  ...
  render() {
    ...
    return (
      <ScrollView style={{ backgroundColor:"#eceff1"}}>
        ...
        style={{width:(30*windowWidth)/100,marginLeft:(3*windowWidth)/100, backgroundColor:'#ffa726',marginTop:10}}>
          <View style={{flex:1}}>
            <ImageBackground
...
export default MyStoreVideoScreen;
